...
class TaskGenerator:
    ...
    def generate_all(self):
        ...
        # --- Level 1: one loop over all 5 types, repeated 5 times ---
        # Order: Poly_1, Rect_1, Circle_1, Star_1, Leaf_1, Poly_2, Rect_2, ...
        for _ in range(5):
            # Polygon
            n = random.choice([3, 4, 5, 6, 7, 8, 9, 10])
            s = random.uniform(30, 100); c = random.choice(COLORS)
            ...
            self._save(self._get_id("L1_Rect") + ".png", 1,
                       ...
                       {"type": "rect", "w": w, "h": h, "color": c})

            # Circle
            s = random.uniform(40, 120); c = random.choice(COLORS)
            x, y = self._rand_pos(s)
            self.t.penup(); self.t.goto(x, y); self.t.pendown()
            draw_circle(self.t, s, c)
            self._save(self._get_id("L1_Circle") + ".png", 1,
                       f"Draw a {c} Circle size {int(s)} at ({int(x)},{int(y)}).",
                       {"type": "circle", "size": s, "color": c})

            # Star
            s = random.uniform(40, 120); c = random.choice(COLORS)
            x, y = self._rand_pos(s)
            self.t.penup(); self.t.goto(x, y); self.t.pendown()
            draw_star(self.t, s, c)
            self._save(self._get_id("L1_Star") + ".png", 1,
                       f"Draw a {c} Star size {int(s)} at ({int(x)},{int(y)}).",
                       {"type": "star", "size": s, "color": c})
            ...
            self._save(self._get_id("L1_Leaf") + ".png", 1,
                       ...
                       {"type": "leaf", "size": s, "angle": ang, "color": c})

       

        # --- Level 2: one loop over all 24 types, repeated 5 times ---
        # Order: House_1, Badge_1, ...(all L2 types)..., House_2, Badge_2, ...
        for _ in range(5):
            # 1. House
            ...
            self.t.penup(); self.t.goto(x, y); self.t.pendown()
            draw_framed_star(self.t, s, c)
            self._save(self._get_id("L2_FrameStar")+".png", 2, f"Framed Star size {int(s)} {c}", {"type":"framed_star"})

            # 19. Door
            w, h = random.uniform(40, 80), random.uniform(80, 140); c = random.choice(COLORS)
            x, y = self._rand_pos(h)
            self.t.penup(); self.t.goto(x, y); self.t.pendown()
            draw_door(self.t, w, h, c)
            ...
            self.t.penup(); self.t.goto(x, y); self.t.pendown()
            draw_flower_pot(self.t, s)
            self._save(self._get_id("L2_Pot")+".png", 2, f"Flower Pot size {int(s)}", {"type":"pot"})

            # 23. Dragonfly
            s = random.uniform(60, 120); x, y = self._rand_pos(s)
            self.t.penup(); self.t.goto(x, y); self.t.pendown()
            draw_dragonfly(self.t, s)
            self._save(self._get_id("L2_Dragonfly")+".png", 2, f"Dragonfly size {int(s)}", {"type":"dragonfly"})

        # --- Level 3: one loop over all 5 scene types, repeated 5 times ---
        # Order: Village_1, Garden_1, ...(all L3)..., Village_2, Garden_2, ...
        for _ in range(5):
            # Village
