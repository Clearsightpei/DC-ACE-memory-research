...
## p2_radical_011_匕   (bootstrap batch, retry_n=2, retry FAILED B1)
...
## p2_radical_015_刀   (bootstrap batch, retry_n=2, retry FAILED B1)
...
## p2_radical_025_力   (batch B1, retry_n=0)

**Attempt file**: `attempts/p2_radical_025_力/01_力.png`

**Diagnosis**: reads as 几 with a stray dot below. 力 = 横折钩 + 撇.
The 撇 in the attempt is a tiny dot near the bottom, not a full 撇
that crosses through the top-left area. The 横折钩 is present.

**Fix for retry**: 撇 must be a full ~150 px stroke starting at the
top-left area of the 横折钩 (near the 横's left third) and throwing
down to the lower-left corner. Its top should reach ABOVE the 横 (same
crossing-visibility rule as 刀). Use taper r=8→1.5.

---

## p2_radical_028_人   (batch B1, retry_n=0)

**Attempt file**: `attempts/p2_radical_028_人/01_人.png`

**Diagnosis**: reads as ∧ (both legs ruler-straight, meeting at apex).
Canonical 人 = 撇 (upper-right→lower-left, gentle bow) + 捺 (upper-left
→lower-right, thin→thick with broad terminal press). The attempt shows
two straight tapered legs with no 捺 press-foot on the right, so it
reads as 入/人 confused. Also, small artifact at right-foot base.

**Fix**: right stroke must be a proper 捺 — start thin at apex, THICKEN
toward the lower-right end, terminate with a broad flat foot (~10 px
radius terminal dab). Left 撇 gets a subtle rightward bow (Bezier ctrl
pulled right).

---

## p2_radical_030_入   (batch B1, retry_n=0)
...
## p2_radical_047_飞   (batch B1, retry_n=0)
...
## p2_radical_048_干   (batch B1, retry_n=0)
...
## p2_radical_053_己   (batch B1, retry_n=0)
...
## p2_radical_058_马   (batch B1, retry_n=0)
...
## p2_radical_067_士   (batch B1, retry_n=0)
