...
**B1 update (post-batch calibration)**: TR11 compliance did NOT
correlate with pass rate on the main batch (63% pass on TR11-compliant
vs 74% pass on non-TR11-compliant). Naming true agreements is honest
labor and stays in the rule (it is the right anti-rubber-stamp
discipline), but it is NOT sufficient for pass prediction — you can
name two true agreements and still fail on layout, proportion, or
stroke-count mismatch. The retry sub-batch skipped TR11 entirely and
still hit 4/6 pass, because mechanical errata-fixes carried them.
Keep TR11 as an epistemic-honesty check; do not treat it as a pass
predictor.

### TR12. Horizontal endpoints share a cell ROW; vertical endpoints
share a cell COLUMN

**Every 横 stroke's head and tail must sit in the same cell row**:
rows are {TL, TC, TR}, {ML, C, MR}, or {BL, BC, BR}. Mixing rows
(e.g. head in BL and tail in C) tilts the "horizontal" by exactly
one cell height (100 px on a 300 canvas) and it renders as a
diagonal, not a horizontal.

Same rule for 竖: both endpoints must share a cell column
({TL, ML, BL}, {TC, C, BC}, or {TR, MR, BR}).

Empirical: batch B1 — 彐 revision moved s3 to a cell-row-mismatched
pair and the "horizontal" came out tilted 100 px, contributing to
the FAIL.

Add to TR8 sanity checks: assert `row(head) == row(tail)` for every
横, and `col(head) == col(tail)` for every 竖, before rendering.
