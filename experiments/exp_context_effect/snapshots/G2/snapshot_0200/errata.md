...
## p2_radical_133_止   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis**: 止 = 卜 top + 一 + 竖 + 一 bottom. Sibling risk with 上 (which lacks the 卜 top-detail). Needs specific stroke ordering.

**Fix**: 4 strokes. Top 卜 = short 竖 + right-side 点 at mid-height. Then a 竖 through the middle. Then a bottom 一 wide. Silhouette square.
...
## p3_char_0009_了   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis**: 了 = 横撇 top + 竖钩 body. Sibling risk with 子 (adds a 一) and 孑 (adds a 一 with hook detail).
...
## p3_char_0011_人   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis (vision-verified)**: Drawer explicitly cited form_catalog AND sibling-pair table but then OVER-REASONED to "small gap between strokes based on handwritten GT" and broke the apex-shared signature. Result reads as 入 (捺 overhangs) or ambiguous.
...
## p3_char_0016_乃   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis**: 乃 = 横折折撇 + 撇. Zigzag body must be tall enough and terminal 撇 dominant — likely too compressed (same failure as p1_stroke_32_横折折折钩).
...
## p3_char_0021_几   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis**: 几 = 撇 + 横折弯钩. Bracket-family. Right side rendered without the sweeping hook.
...
## p3_char_0023_九   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis**: 九 = 撇 + 横折弯钩. Same failure as 几 — the terminal sweeping-hook underscaled.

**Fix**: Similar to 几 fix; the 撇 is shorter and higher; the 横折弯钩 dominates and sweeps wider (~200 px x-extent).

---

## p3_char_0029_入   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis (recurring)**: 入 needs 捺 overhanging the 撇 (~30 px offset). Attempt renders as 人-clone. Recurring signature-bit failure across batches.

**Fix**: 捺 starts at y=50, 撇 starts at y=90 (30 px offset minimum). The 捺's top must be VISIBLY above and left of the 撇's top. Cross-ref form_catalog sibling-pair table row 人-vs-入.

---

## p3_char_0033_刀   (batch B3, retry_n=0, initial_batch: B3)

**Diagnosis (recurring)**: 刀 = 横折钩 + body-crossing 撇 (same as 力 but no 力-hook body). Recurring crossing-visibility failure across bootstrap/B1/B2/B3.
